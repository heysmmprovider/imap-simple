...
function ImapSimple(imap) {
    ...
    self.imap.on('error', function (err) {
        ...
        if (err && self.ending && (err.code && ['ECONNRESET', 'EPIPE'].includes(err.code.toUpperCase()))) {
            console.log("Suppressed error after connection end:", err.code);
            return; // Do not propagate this error since it's a known state during disconnection
        }
